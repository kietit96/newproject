 ...
 * To edit the configs it is best to create a file called resizeImage-config.php
 * and define variables you want to customize in there. It will automatically be
 * loaded by resizeImage. This will save you having to re-edit these variables
 * everytime you download a new version
*/
define ('VERSION', '2.8.14');																		// Version of this script 
//Load a config file if it exists. Otherwise, use the values below
if( file_exists(dirname(__FILE__) . '/resizeImage-config.php'))	require_once('resizeImage-config.php');
...
if(! defined('FILE_CACHE_SUFFIX') ) 		define ('FILE_CACHE_SUFFIX', '.resizeImage.txt');			// What to put at the end of all files in the cache directory so we can identify them
if(! defined('FILE_CACHE_PREFIX') ) 		define ('FILE_CACHE_PREFIX', 'resizeImage');				// What to put at the beg of all files in the cache directory so we can identify them
if(! defined('FILE_CACHE_DIRECTORY') ) 		define ('FILE_CACHE_DIRECTORY', './cacheImages');				// Directory where images are cached. Left blank it will use the system temporary directory (which is better for security)
...
if(! defined('DEFAULT_Q') )					define ('DEFAULT_Q', 90);								// Default image quality. Allows overrid in resizeImage-config.php
if(! defined('DEFAULT_ZC') )				define ('DEFAULT_ZC', 1);								// Default zoom/crop setting. Allows overrid in resizeImage-config.php
if(! defined('DEFAULT_F') )					define ('DEFAULT_F', '');								// Default image filters. Allows overrid in resizeImage-config.php
if(! defined('DEFAULT_S') )					define ('DEFAULT_S', 0);								// Default sharpen value. Allows overrid in resizeImage-config.php
if(! defined('DEFAULT_CC') )				define ('DEFAULT_CC', 'ffffff');						// Default canvas colour. Allows overrid in resizeImage-config.php
if(! defined('DEFAULT_WIDTH') )				define ('DEFAULT_WIDTH', 100);							// Default thumbnail width. Allows overrid in resizeImage-config.php
if(! defined('DEFAULT_HEIGHT') )			define ('DEFAULT_HEIGHT', 100);							// Default thumbnail height. Allows overrid in resizeImage-config.php

/**
 * Additional Parameters:
 * LOCAL_FILE_BASE_DIRECTORY = Override the DOCUMENT_ROOT. This is best used in resizeImage-config.php
 */
...
/*
	...
	10. http://yoursite.com/path/to/resizeImage.php?src=http://markmaunder.com/&webshot=1

	Notes on performance: 
	The first time a webshot loads, it will take a few seconds.
	From then on it uses the regular resizeImage caching mechanism with the configurable options above
...
class resizeImage {
	...
	protected $salt = "";
	protected $fileCacheVersion = 1; //Generally if resizeImage.php is modifed (upgraded) then the salt changes and all cache files are recreated. This is a backup mechanism to force regen.
	...
	public function __construct(){
		...
		if($this->isURL){
			if(ALLOW_ALL_EXTERNAL_SITES){
				...
				if(! $allowed){
					return $this->error("You may not fetch images from that site. To enable this site in resizeImage, you can either add it to \$ALLOWED_SITES and set ALLOW_EXTERNAL=true. Or you can set ALLOW_ALL_EXTERNAL_SITES=true, depending on your security needs.");
				}
			}
		}
	...
	protected function serveErrors(){
		...
		$html .= '</ul>';
		echo '<h1>A ResizeImage error has occured</h1>The following error(s) occured:<br />' . $html . '<br />';
		echo '<br />Query String : ' . htmlentities( $_SERVER['QUERY_STRING'], ENT_QUOTES );
		echo '<br />ResizeImage version : ' . VERSION . '</pre>';
	}
	...
	protected function cleanCache(){
		...
		$lastCleanFile = $this->cacheDirectory . '/resizeImage_cacheLastCleanTime.touch';
		
		//If this is a new resizeImage installation we need to create the file
	...
	protected function processImageAndWriteToCache($localImage){
		...
		$imgType = "";
		$tempfile = tempnam($this->cacheDirectory, 'resizeImage_tmpimg_');
		...
		if($imgType == 'png' && OPTIPNG_ENABLED && OPTIPNG_PATH && @is_file(OPTIPNG_PATH)){
			...
			$exec = PNGCRUSH_PATH;
			$tempfile2 = tempnam($this->cacheDirectory, 'resizeImage_tmpimg_');
		...
		$tempfile4 = tempnam($this->cacheDirectory, 'resizeImage_tmpimg_');
	...
	protected function getLocalImagePath($src){
		...
		if(! $this->docRoot){
			...
			return $this->error("Could not find your website document root and the file specified doesn't exist in resizeImages directory. We don't support serving files outside resizeImage's directory without a document root for security reasons.");
	...
	protected function serveWebshot(){
		...
		$instr = "Please follow the instructions at http://code.google.com/p/resizeImage/ to set your server up for taking website screenshots.";
		...
		$tempfile = tempnam($this->cacheDirectory, 'resizeImage_webshot');
		$url = $this->src;
	...
	protected function serveExternalImage(){
		...
		$tempfile = tempnam($this->cacheDirectory, 'resizeImage');
	...
	protected function serveCacheFile(){
		...
		if(! is_file($this->cachefile)){
			$this->error("serveCacheFile called in resizeImage but we couldn't find the cached file.");
			return false;
		}
	...
	protected function debug($level, $msg){
		if(DEBUG_ON && $level <= DEBUG_LEVEL){
			...
			error_log("ResizeImage Debug line " . __LINE__ . " [$execTime : $tick]: $msg");
		}
	}
	protected function sanityFail($msg){
		return $this->error("There is a problem in the resizeImage code. Message: Please report this error at <a href='http://code.google.com/p/resizeImage/issues/list'>resizeImage's bug tracking page</a>: $msg");
	}
	...
	protected function setMemoryLimit(){
		...
		$inibytes = resizeImage::returnBytes($inimem);
		$ourbytes = resizeImage::returnBytes(MEMORY_LIMIT);
	...
	protected function getURL($url, $tempfile){
		...
		if(function_exists('curl_init')){
			...
			curl_setopt ($curl, CURLOPT_WRITEFUNCTION, 'resizeImage::curlWrite');
