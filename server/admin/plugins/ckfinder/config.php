...
function CheckAuthentication()
{
	global $db;
	global $_COOKIE;
	$login = $db->alone_data_where_where('user','password',md5($_COOKIE['password']),'email',$_COOKIE['user']);
...
if(isset($_GET['resize_w']) && isset($_GET['resize_h']) ) {
    $config['Images']['maxWidth'] = $_GET['resize_w'];
    $config['Images']['maxHeight'] = $_GET['resize_h'];
}
